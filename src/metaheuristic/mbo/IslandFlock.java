...
public class IslandFlock {
    ...
    public int getWorstIndex() {
        int worstIndex =0;
        Bird worst = birds.get(0);

        for (int i=0;i<birds.size();i++)
        {
            if (birds.get(i).getIndividual().getCost()>worst.getIndividual().getCost())
            {
            	worst= birds.get(i);
                worstIndex = i;
            }
    ...
    void generateNeighbors(RNG rng, OptimizationProblem problem, List<MutationOperator> neighboringFunctions, int nNeighbors, int nShared)
    {
        ...
        {
            Individual neighbor = iLeader.clone();
        ...
        //Arrays.parallelSort(leader.getNeighbours(),new CostBasedComparator());
        ...
        for (int i = 0; i < birds.size(); i++) {
            ...
            {
                Individual neighbor = iBird.clone();
         ...
         //Arrays.parallelSort(b.getNeighbours(),new CostBasedComparator());
