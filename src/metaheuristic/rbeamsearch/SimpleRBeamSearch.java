...
public class SimpleRBeamSearch extends AbstractSMetaheuristic {

    private int ROULETTE_SIZE = 10;
    private int BEAM_SIZE = 5;
    private int depth = 1;
    ...
    public SimpleRBeamSearch() {
    }

    public void setROULETTE_SIZE(int ROULETTE_SIZE) {
        this.ROULETTE_SIZE = ROULETTE_SIZE;
    }

    public void setBEAM_SIZE(int BEAM_SIZE) {
        this.BEAM_SIZE = BEAM_SIZE;
    }

    public SimpleRBeamSearch(int releaseCount, int depth, int maxIterationCount, RBeamChildGenerator childGenerator, ReleaseHandler releaseHandler, TerminalCondition terminalCondition) {
        this.depth = depth;
    ...
    public SimpleRBeamSearch(SimpleRBeamSearch rBeamSearch) {
        super(rBeamSearch);
        this.depth = rBeamSearch.depth;
        this.releaseCount = rBeamSearch.releaseCount;
        this.maxIterationCount = rBeamSearch.maxIterationCount;
        if (rBeamSearch.childGenerator != null)
            this.childGenerator = rBeamSearch.childGenerator.clone();
        if (rBeamSearch.releaseHandler != null)
            this.releaseHandler = rBeamSearch.releaseHandler.clone();
        this.BEAM_SIZE = rBeamSearch.BEAM_SIZE;
        this.ROULETTE_SIZE = rBeamSearch.ROULETTE_SIZE;
    }
    ...
    protected void _perform(OptimizationProblem problem, InitialSolutionGenerator solutionGenerator) {
        ...
        while (!terminalCondition.isSatisfied(this,problem))
        {
            ...
            {
                if (!releaseHandler.release(problem,currentSolution,releaseCount)) {
                    System.out.println("AHAAAA");
                    break;
                }
                beamSearch(problem,currentSolution);
        ...
        //System.out.println("SRBS-IC"+iterationCount);
    }

    @Override
    public AbstractSMetaheuristic clone() {
        AbstractSMetaheuristic clone = new SimpleRBeamSearch(this);
        return clone;
    }
    ...
    public static MetaHeuristic createInstance(OptimizationProblem problem, String[] params) {
        SimpleRBeamSearch simpleRBeamSearch = new SimpleRBeamSearch();

        simpleRBeamSearch.releaseCount = Integer.parseInt((params[0]));
        simpleRBeamSearch.depth = Integer.parseInt(params[1]);
        simpleRBeamSearch.childGenerator = new MOTAPRBeamChildGenerator();
        simpleRBeamSearch.releaseHandler = new BasicRelease();
        simpleRBeamSearch.terminalCondition = EAService.TerminalConditions.createTerminalCondition(params[2],problem);

        return simpleRBeamSearch;
